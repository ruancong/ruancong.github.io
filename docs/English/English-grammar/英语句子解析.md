...
It is suggested to plan your selectors up front.

> up front: 副词短语，它的核心意思是：
>
> in advance; before you start something
>
> 语法难点解析 (Grammar Point Explanation)
>
> - **It as a Formal Subject (形式主语)**：当一个句子的主语是一个很长的不定式短语 (`to do something`) 或从句时，为了避免头重脚轻，我们常常用 "it" 放在句首，而把真正的主语移到后面。
>   - *例如：* `To learn English well is important.`  可以写成 `It is important to learn English well.` (学好英语很重要。) 你的例句也是同样的结构。
> - **Passive Voice (被动语态)**：`is suggested` 强调的是“建议”这个动作本身，而不是谁发出的建议。这在提出指导性意见时非常常见。在很多技术文档或正式建议中，使用被动语态可以显得更客观，避免直接使用 "we" 或 "I"。
...
> **第一个 `When`** 引出的是**状语**（修饰动作的时间），回答了主句动作“*何时*发生？”。
>
> **第二个 `When`** 引出的是**宾语**（动作 `define` 的内容/对象），回答了谓语动词“*定义了什么*？”。

### 29.

The boss likes everything spick and span in the office.

> spick and span 通常作为一个整体形容词短语使用，很少拆开。它的意思是：极干净的、整洁洁新的、焕然一新的。
>
> spick and span 做为形容词短语做宾语补语，修饰宾语 `everything`。
...
> 这里有一个非常容易让人迷惑的地方！正常的英语语序应该是 **apply A to B** (把A应用到B)。 按照正常语序，这句话应该是：*to apply [Spring ecosystem design principles...] to [the AI domain]*. 但是，因为宾语 A (*Spring ecosystem design principles such as portability and modular design*) 太长了，为了避免“头重脚轻”，作者把状语 `to the AI domain` 提到了前面，变成了 **apply to B A** 的结构。阅读时一定要敏锐地识别出 `apply` 的真正宾语是谁。